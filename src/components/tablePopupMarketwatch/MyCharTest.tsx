import React, { useContext, useEffect, useState } from "react";
import * as Highcharts from "highcharts";
import { ChartContext } from "./ChartWithOption";
import {
  _getDateTs,
  drawChartOption,
  getTimeString,
  maxNumber,
  minNumber,
} from "../chartIndex/util/app.chart";
import { formatNumbertoDecimal, formatNumber } from "../../utils/util";
import { DataReponse } from "./TablePopupMarketwatch";

const useChart = (dataChartOption: DataReponse, indexValue: any) => {
  const { type } = useContext(ChartContext);
  const [dataCol, setDataCol] = useState<any>([]);
  const [dataSpline, setDataSpline] = useState<any>([]);
  console.log({ dataChartOption });

  const arrPr = React.useMemo(() => {
    if (dataChartOption.length === 0) {
      return [];
    }
    const arr = drawChartOption(dataChartOption, type).map((item: any) => {
      return item[4];
    });
    return arr;
  }, [dataChartOption, type]);

  useEffect(() => {
    if (dataChartOption?.length !== 0) {
      const data = drawChartOption(dataChartOption, type)?.map((item: any) => ({
        TimeJS: _getDateTs(item[0]),
        Index: item[4],
        Vol: item[5],
      }));
      const arrSpline = data?.map((item: any) => {
        return {
          x: item?.TimeJS,
          y: item?.Index,
        };
      });
      const arrCol = data?.map((item: any) => ({
        x: item?.TimeJS,
        y: item?.Vol,
      }));
      setDataCol(arrCol);
      setDataSpline(arrSpline);
    }
  }, [dataChartOption, type]);
  console.log({dataCol,dataSpline});
  

  const gradientColor: any = [0, 0, 50, 500];
  const seriesOptions: Highcharts.SeriesOptionsType[] = [
    {
      name: "Column",
      type: "column",
      yAxis: 0,
      states: {
        hover: {
          enabled: false,
        },
      },
      data: dataCol,
    },
    {
      name: "Spline",
      type: type === "a1d" ? "spline" : "line",
      yAxis: 1,
      data: dataSpline,
      color: "#00FF00",
    },
  ];
  const config: Highcharts.Options = {
    boost: {
      enabled: true,
    },
    chart: {
      // type: "spline",
      polar: true,
      height: 160,
      backgroundColor: "#303030",
      plotBackgroundColor: {
        linearGradient: gradientColor,
        stops: [
          [0, "#080808"],
          [1, "#917c05"],
        ],
      },
      plotBorderWidth: 1,
      plotBorderColor: "#545454",
      events: {
        load: function () {
          this.yAxis[1].update({
            labels: {
              format: "{value:.1f}",
            },
          });
          const xAxis = this.xAxis[0];
          const today = new Date();
          const dd = today.getDate();
          const mm = today.getMonth(); //January is 0!
          const yyyy = today.getFullYear();
          const HH1 = 9;
          const HH2 = 15;
          const MM = 0; // minute

          const xminTmp = new Date(yyyy, mm, dd, HH1, MM);
          const xmaxTmp = new Date(yyyy, mm, dd, HH2, MM);

          const xmin = _getDateTs(xminTmp);
          const xmax = _getDateTs(xmaxTmp);
          xAxis.setExtremes(xmin, xmax, true, false);
          switch (type) {
            case "a1d":
              this.xAxis[0].update({
                tickInterval: 3600 * 1000,
              });
              xAxis.setExtremes(xmin, xmax, true, false);
              this.redraw();
              break;
            case "a1w":
              this.xAxis[0].update({
                labels: {
                  formatter: function () {
                    const day = new Date(this.value).getDate();
                    const month = new Date(this.value).getMonth() + 1;
                    return `${day > 9 ? day : `0${day}`}/${
                      month > 9 ? month : `0${month}`
                    }`;
                  },
                },
                // startOfWeek: 1,
                // tickInterval: 24 * 3600 * 1000,
              });
              this.redraw();

              break;
            case "a3m":
            case "a6m":
              this.xAxis[0].update({
                labels: {
                  formatter: function () {
                    const day = new Date(this.value).getDate();
                    const month = new Date(this.value).getMonth() + 1;
                    return `${day > 9 ? day : `0${day}`}/${
                      month > 9 ? month : `0${month}`
                    }`;
                  },
                },
                // startOfWeek: 1,
                tickInterval: 30 * 24 * 3600 * 1000,
              });
              break;
            case "a1y":
            case "a2y":
              this.xAxis[0].update({
                labels: {
                  formatter: function () {
                    const year = new Date(this.value).getFullYear();
                    const month = new Date(this.value).getMonth() + 1;
                    return `${month > 9 ? month : `0${month}`}/${year}`;
                  },
                },
              });
              break;
            default:
              this.xAxis[0].update({
                tickInterval: 3600 * 1000,
              });
              xAxis.setExtremes(xmin, xmax, true, false);
              break;
          }
          let price_min, price_max: number, tick, minSub;
          let barwidth: number;
          let arrSub: any = [];
          let tickPosition: any = [];

          price_min = minNumber(arrPr);
          price_max = maxNumber(arrPr);

          if (arrPr.length === 0) {
            const calculateMinMax = (
              num: number
            ): { min: number; max: number } => {
              let min = Math.floor(num * 10) / 10;
              let max = min + 0.1;
              if (min === indexValue) {
                min = indexValue - 0.1;
                max = indexValue + 0.1;
              }
              return { min, max };
            };
            const { max, min } = calculateMinMax(indexValue);

            let tickPosition: any = [];
            for (let i = min; i <= max; i += 0.1) {
              tickPosition.push(i);
            }
            this.yAxis[1].update({
              tickPositions: tickPosition,
            });
          } else {
            for (let i = 0; i < arrPr.length; i++) {
              let next = arrPr[i + 1];

              if (typeof next === "undefined") {
                next = arrPr[0];
                arrSub.push(Math.abs(Math.round((arrPr[i] - next) * 10) / 10));
              }
            }
            minSub = minNumber(arrSub);
            if (price_min > indexValue) {
              tick = 0.1;
              price_min = indexValue - tick;
              price_max += tick;
              barwidth = 0.05;
            } else {
              if (price_max < 50) {
                tick = 0.1;
                barwidth = 0.05;
                price_min -= tick;
                price_max += tick;
              } else if (price_max > 100) {
                tick = minSub < 0.5 ? 0.1 : 0.5;
                barwidth = 0.05;
                price_min -= tick;
                price_max += tick;
              } else if (minSub < 0.5) {
                tick = 0.1;
                barwidth = 0.05;
                price_min -= tick;
                price_max += tick;
              } else {
                tick = 1;
                barwidth = 0.2;
                price_min -= tick;
                price_max += tick;
              }
            }
            let sub = price_max - price_min;
            if (sub > 0) {
              let countTick = Math.round(sub / tick);
              if (countTick > 20) {
                let tempTick = Math.round(countTick / 15);
                tick = 0.5;
                barwidth = 0.05;
                price_min -= tick;
                price_max += tick;
              }
            }

          //   // eslint-disable-next-line array-callback-return
            this.series[0].points.map((e) => {
              e.update({
                pointWidth: 0.1,
              });
            });

            for (
              let i =
                type !== "a1d" ? Math.floor(price_min * 10) / 10 : price_min;
              i <=
              (type !== "a1d"
                ? Math.ceil(price_max * 10) / 10
                : Math.floor(price_max * 10) / 10);
              i += tick
            ) {
              tickPosition.push(Number(parseFloat(i.toFixed(2))));
            }

            this.yAxis[1].update({
              tickPositions: tickPosition,
            });
          }
          this.redraw();
        },
      },
    },
    credits: {
      enabled: false,
    },
    title: {
      text: "",
    },
    xAxis: {
      type: "datetime",
      dateTimeLabelFormats: {
        hour: "%H h",
      },
      labels: {
        useHTML: true,
        style: {
          color: "#a5a5a5",
          fontSize: "6pt",
        },
        align: "center",
        y: 15,
      },
      lineWidth: 0,
      lineColor: "#5f5f5f",
      tickWidth: 0,
      // tickInterval: 3600000,
      gridLineWidth: 1,
      gridLineColor: "#6d6d6d1f",
      height: 140,
      endOnTick: false,
      startOnTick: true,
      startOfWeek: 0,
    },
    yAxis: [
      {
        title: {
          text: "",
        },
        gridLineWidth: 0,
        opposite: true,
        lineWidth: 0,
        lineColor: "#5f5f5f",
        labels: {
          enabled: false,
        },
        height: 140,
      },
      {
        title: {
          text: "",
        },
        endOnTick: true,
        lineWidth: 0,
        lineColor: "#5f5f5f",
        labels: {
          style: {
            fontSize: "6pt",
            color: "#a5a5a5",
          },
          distance: 6,
          y: 2,
        },
        gridLineWidth: 1,
        gridLineColor: "#6d6d6d1f",
        height: 140,
        plotLines:
          type === "a1d"
            ? [
                {
                  color: "#FFFF00",
                  width: 1,
                  value: indexValue,
                  zIndex: 10,
                },
              ]
            : [],
      },
    ],
    time: {
      useUTC: false,
    },
    tooltip: {
      shared: true,
      shadow: false,
      backgroundColor: "#ffffffc9",
      borderColor: "#07d800",
      borderRadius: 5,
      borderWidth: 1,
      padding: 6,
      useHTML: true,
      formatter: function () {
        const index: any = this.points?.map((e: any, ind) => {
          if (ind === 1) {
            if (type === "a1d") {
              if (e.y >= indexValue) {
                e.series.chart.tooltip.options.borderColor = "#07d800";
              } else {
                e.series.chart.tooltip.options.borderColor = "red";
              }
            }
            return { x: e.x, y: e.y };
          }
          return "";
        });
        const hour: any = new Date(Number(index[1].x)).getHours();
        const minutes =
          new Date(Number(index[1].x)).getMinutes().toString().length === 1
            ? "0" + new Date(Number(index[1].x)).getMinutes()
            : new Date(Number(index[1].x)).getMinutes();

        return `<span style="color:#000">Thời gian: <b style="font-size:12px;font-weight:600;color:#000" class="font-bold text-sm">${
          type !== "a1d" ? getTimeString(index[1].x) : hour + ":" + minutes
        }</b></span><br/><span style="color:#000">Index:  <b style="font-size:12px;color:#000" class="font-bold text-sm">${
          index[1].y
        }</b></span><br/><span style="color:#000">Khối lượng: <b style="font-size:12px;color:#000" class="font-bold text-sm">${
          type !== "a1d" ? formatNumbertoDecimal(this.y) : formatNumber(this.y)
        } </b></span>`;
      },
      positioner: function (labelWidth, labelHeight, point) {
        var tooltipX, tooltipY;
        // Calculate tooltip X position
        if (point.plotX + labelWidth > this.chart.plotWidth) {
          tooltipX = point.plotX - labelWidth + this.chart.plotLeft - 10;
        } else if (point.plotX - labelWidth < 0) {
          tooltipX = point.plotX + this.chart.plotLeft + 10;
        } else {
          tooltipX = point.plotX + this.chart.plotLeft - labelWidth / 2;
        }
        // Calculate tooltip Y position
        if (point.plotY + labelHeight > this.chart.plotHeight) {
          tooltipY = point.plotY - labelHeight + this.chart.plotTop - 10;
        } else if (point.plotY - labelHeight < 0) {
          tooltipY = point.plotY + this.chart.plotTop + 10;
        } else {
          tooltipY = point.plotY + this.chart.plotTop - labelHeight - 10;
        }

        return {
          x: tooltipX,

          y: tooltipY,
        };
      },
    },
    legend: {
      enabled: false,
    },
    plotOptions: {
      spline: {
        color: "#00FF00",
        lineWidth: 1.5,
        zones:
          type !== "a1d"
            ? []
            : [
                {
                  value: indexValue,
                  color: "red",
                },
                {
                  color: "#00FF00",
                },
              ],
      },
      line: {
        color: "#00FF00",
        lineWidth: 1,
        marker: {
          enabled: false,
        },
      },
      column: {
        color: "#5F9DFE",
        borderWidth: 0,
        borderRadius: 0,
      },
    },
    series: seriesOptions,
  };
  return { config };
};

export default useChart;
