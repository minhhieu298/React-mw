...
export type DataReponse = (string | number)[];

type DataContext = {
  data: DataReponse;
  param: string;
  updateParam: (option: string) => void;
};

export const TablePopupContext = React.createContext<DataContext | undefined>(
  undefined
);

export const useTableContext = () => {
  const context = useContext(TablePopupContext);
  if (!context) {
    throw Error;
  }
  return context;
...
const TablePopupMarketwatch = () => {
  ...
  const [dataChart, setDataChart] = useState<DataReponse>([]);
  const [param, setParam] = useState<string>("gw_realtime");
  ...
  return (
    <Draggable handle=".pu-header" position={position} onDrag={handleDrag}>
      <div className="pu-window text-[#B9B9B9]">
        ...
        <TablePopupContext.Provider
          value={{
            data: dataChart as DataReponse,
            param: param,
