...
const BASE_URL = "http://eztrade4.fpts.com.vn/";
...
const ListDataTable = {
  list: (floor: string, valueParam: string) =>
    requests.get(
      `http://eztrade4.fpts.com.vn/${floor}/data.ashx?${valueParam}`
    ),
};
const dataGDTTtable = {
  listPt: (floor: string) =>
    requests.get(`http://eztrade4.fpts.com.vn/${floor}/data.ashx?s=pt`),
  listBi: (floor: string) =>
    requests.get(`http://eztrade4.fpts.com.vn/${floor}/data.ashx?s=bi`),
};
const chartIndex = {
...
const tableDetailPopup = {
  get: (params: any) =>
    requests.get(
      `${BASE_URL}/hsx/data.ashx?s=quote&l=${params}`
    ),
};
